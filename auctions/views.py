...
from auctions.models import Contact
from django.contrib import messages

from .models import Auction, Bid
from .forms import ImageUploadForm
from Testing import get
import numpy as np
import pandas as pd
...
def index2(request):
    ...
    # Get all active auctions, oldest first
    latest_auction_list = auction_list.filter(is_active=True).order_by('date_added')
    template = loader.get_template('auctions/index2.html')
    context = {
        'title': "Active auctions",
        'auction_list': latest_auction_list,
    }
...
def create(request):
    ...
    if submit_button:
        try:
            ...
            AUCTION_DURATION = request.POST['AUCTION_DURATION']
...
def crop_prediction(request):
    ...
    for s in X1:
        soil_type.append(s.replace(' ', '_'))

    soil_list = unique(soil_type)

    if request.method == 'GET':
        return render(request, 'auctions/crop_prediction.html', {'soil_list': soil_list})
    else:

        selected_soil = request.POST['selected_soil']
        T = request.POST['T']
        WS = request.POST['WS']
        ...
        try:
            ...
            K = float(K)
            print(selected_soil)
            selected_soil = selected_soil.replace('_', ' ')
            result = get(T, WS, pre, H, N, P, K, selected_soil)

            return render(request, 'auctions/crop_prediction.html', {'result': result, 'T': T, 'WS': WS, 'pre': pre, 'H': H, 'N': N, 'P': P, 'K': K,
                           'selected_soil': selected_soil})
        except:
            return render(request, 'auctions/crop_prediction.html', {'error_message': 'Please consider all the fields correctly', 'soil_list': soil_list})
