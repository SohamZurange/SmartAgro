...
class Auction(models.Model):
    ...
    AUCTION_DURATION = models.IntegerField(null=True)
    ...
    def resolve(self):
        if self.is_active:
            ...
            if self.has_expired():
                # Define winner
                highest_bid = Bid.objects.filter(auction=self).order_by('date').order_by('-amount').first()
                if highest_bid:
    ...
    def has_expired(self):
        ...
        expiration = self.date_added + timedelta(minutes=self.AUCTION_DURATION)
        if now > expiration:
            return True
        else:
            return False
